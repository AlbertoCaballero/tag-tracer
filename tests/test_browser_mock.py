...
def mock_playwright(mocker):
    ...
    mocker.patch(
        "tag_tracer.browser.browser.async_playwright",
    ...
    mock_playwright_context.start.return_value = (
        mock_playwright_context  # It returns itself when awaited
    )

    # Return the specific mocks that tests will assert against
    return mock_playwright_context, mock_browser, mock_context, mock_page


@pytest.mark.asyncio
async def test_browser_manager_launch(mock_playwright):
    _, mock_browser, mock_context, mock_page = mock_playwright
...
async def test_browser_manager_navigate(mock_playwright):
    ...
    mock_page.goto.assert_called_once_with(
        test_url, wait_until="domcontentloaded", timeout=60000
    )
...
async def test_browser_manager_close(mock_playwright):
    mock_pw, mock_browser, _, mock_page = mock_playwright
    manager = BrowserManager(headless=True)
    await manager.launch()
    ...
    mock_page.remove_listener.assert_called_once_with(
        "request", manager._handle_request
    )
