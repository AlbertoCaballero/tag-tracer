...
from tag_tracer.config.loader import ExcelConfig, PageConfig, VendorConfig
        ...
        pages=[
            PageConfig(
                ...
                expected_tags={
                    ...
                    },  # Dictionary with value and rules
                    "campaign_id": {
                        "rules": {
                            "regex_match": {"type": "regex", "value": r"^CID-\d+"}
                        }
                    },  # Dictionary with rules only
                    "user_id": {
                        "rules": {"present_check": {"type": "present"}}
                    },  # Dictionary with rules only
                    "optional_param": {
                        "value": "some_value",
                        "rules": {
                            "contains_check": {"type": "contains", "value": "some"}
                        },
                    },  # Dictionary with value and rules
                },
            ),
            PageConfig(
                ...
                expected_tags={
                    ...
                    "item_id": {
                        "value": "123",
                        "rules": {"exact": {"type": "exact", "value": "123"}},
                    },
                },
            ),
        ],
...
def test_get_relevant_requests(
    sample_excel_config, sample_matcher, sample_network_requests
):
    validator = Validator(sample_excel_config, sample_matcher)
    ...
    assert (
        len(relevant) == 4
    )  # GA page_view (home), GA view_item (product), GA user_id, GA optional_param

    # Test for product_page vendors (analytics, marketing)
    relevant = validator._get_relevant_requests(
        sample_network_requests, ["analytics", "marketing"]
    )
    assert (
        len(relevant) == 5
    )  # GA page_view (home), GA view_item (product), Ads track, GA user_id, GA optional_param
...
def test_validate_home_page_all_passed(sample_excel_config, sample_matcher):
    ...
    )  # Only home_page should pass if other pages have no relevant requests
    assert summary.pages_failed == 1

    home_page_result = next(
        (p for p in summary.page_results if p.page_id == "home_page"), None
    )
...
def test_validate_vendor_scoped_tags_missing_vendor(
    multi_vendor_config, sample_matcher
):
...
def test_page_result_exposes_expected_and_found_tags(
    sample_excel_config, sample_matcher
):
...
def test_validate_domain_matching_avoids_substring_false_positive(sample_matcher):
    ...
    summary = validator.validate(
        [
            NetworkRequest(
                url="https://www.notfacebook.com/tr/?ev=ViewContent",
                method="GET",
                headers={},
                post_data=None,
            )
...
def test_page_result_actual_value_not_polluted_across_vendors(sample_matcher):
    ...
    requests = [
        NetworkRequest(
            url="https://a.example.com/track?id=111",
            method="GET",
            headers={},
            post_data=None,
        ),
        NetworkRequest(
            url="https://b.example.com/track?id=222",
            method="GET",
            headers={},
            post_data=None,
        ),
